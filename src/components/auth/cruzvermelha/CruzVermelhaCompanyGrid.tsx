...
interface CruzVermelhaCompanyGridProps {
  companies: Company[];
  selectedCompany: string | null;
  onCompanySelect: (companyId: string) => void;
}

export function CruzVermelhaCompanyGrid({ companies, selectedCompany, onCompanySelect }: CruzVermelhaCompanyGridProps) {
  return (
    <div className="grid grid-cols-3 gap-4 mb-8" role="grid" aria-label="Cruz Vermelha organizations">
      {companies.map((company) => (
        <button
          ...
          aria-label={`Select ${company.name} organization`}
        >
